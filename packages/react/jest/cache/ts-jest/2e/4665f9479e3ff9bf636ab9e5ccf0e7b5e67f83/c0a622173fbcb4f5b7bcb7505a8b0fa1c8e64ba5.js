...
describe('use-debounce', function() {
  test('default value should work', function() {
    var result = react_hooks_1.renderHook(function() {
      return use_debounce_1.useDebounce({ defaultValue: 1 })
    }).result
    expect(result.current.value).toBe(1)
    expect(result.current.debouncedValue).toBe(1)
  })
  test('value should be react, debounced value should be debounced', function() {
        ...
        switch (_b.label) {
          case 0:
            ;(_a = react_hooks_1.renderHook(function() {
              return use_debounce_1.useDebounce({ interval: 2000 })
            })),
              (result = _a.result),
